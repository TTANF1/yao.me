...
## 当前状态
- 最近提交 `858a747`；工作区未提交批次：基础功能 14 文件 + 两轮体验修正（h1 fit-content 防畸变、下划线禁入快照、morph 时长自适应、返回取消过渡、TOC 延后、回顶按钮避让 footer+丝滑过渡、ScrambleText 导航在途跳过洗牌（修重影/乱码误播）、MermaidRenderer 延迟到过渡落定后渲染（修长文掉帧）），tsc/ESLint/浏览器实测通过，**尚未 commit（需用户确认）**
