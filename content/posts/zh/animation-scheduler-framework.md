---
title: 从 setTimeout 地狱到声明式编排：我为 IP 主舞台写的一个动画调度框架
date: '2025-04-01'
summary: 这套框架最终在同一个主舞台上同时驱动了三条独立剧情线，证明了抽象本身的复用价值。本文记录抽象的四个概念、核心执行模型(节点串行 + 节点内并行)、四类真实踩坑，以及六条我从这次抽象里沉淀下来的判断。
ai: true
draft: false
---
